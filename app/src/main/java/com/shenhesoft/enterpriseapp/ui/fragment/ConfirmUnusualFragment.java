 ...
 * @desc TODO
 */
public class ConfirmUnusualFragment extends BaseFragment implements BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    private void submit() {
        Observable<RequestResults<ListALLResults<MotorDetailsBean>>> observable = HttpManager.getInstance().getUserService()
                .waitUnloadingunsual(ApiRetrofit.getInstance().waitUnloadingunusual(pageNo + "","4"));
